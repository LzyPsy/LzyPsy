...
#define IF if(huihe!=1)huihe++;else {huihe=0;speed=speed-2;}
...
void moshi(){//模式选择 
	 ...
	 scanf("%d",&jijie);
	 fflush(stdin);
if(jijie!=1&&jijie!=2&&jijie!=3&&jijie!=4&&jijie!=5)
	  goto menu;
}
...
#define SHANG \
	if(*(zuobiao+SIZE*(x1-1)+y1)=='L'&&(jijie==1||jijie==5)){\
	srand(time(NULL));\
		int zhuan=rand()%2+1;\
	gotoxy(0,0);\
...
gotoxy(0,0); \
  if(zhuan==1){pan=zuo((char *)zuobiao);if(pan==0){pan=1;return 0;}}\
	...
	*(zuobiao+SIZE*x1+y1)='@';\
	...
	else if(*(zuobiao+SIZE*(x1-1)+y1)=='#'||*(zuobiao+SIZE*(x1-1)+y1)=='&'||\
	*(zuobiao+SIZE*(x1-1)+y1)=='*'||*(zuobiao+SIZE*(x1-1)+y1)=='+')return 0;\
	else if(*(zuobiao+SIZE*(x1-1)+y1)=='U') {gotoxy(0,0);	\
				gotoxy(2*y1,x1);\
				*(zuobiao+SIZE*x1+y1)='*';\
				printf("%c",*(zuobiao+SIZE*x1+y1));\
				x1--;\
...
//定义判断向左运动的宏 
#define ZUO \
	if(*(zuobiao+SIZE*x1+y1-1)=='L'&&(jijie==1||jijie==5)){\
		srand(time(NULL));\
		int zhuan=rand()%2+1;\
	gotoxy(0,0);\
		gotoxy(2*y1,x1);*(zuobiao+SIZE*x1+y1)='*';\
...
gotoxy(0,0);\
 if(zhuan==1){pan=xia((char *)zuobiao);if(pan==0){pan=1;return 0;}}\
	...
	bianli(3);\
		...
		eluosiyidong((char *)zuobiao);\
gotoxy(0,0);}\
	else if(*(zuobiao+SIZE*x1+y1-1)=='#'||*(zuobiao+SIZE*x1+y1-1)=='&'||\
	*(zuobiao+SIZE*x1+y1-1)=='*'||*(zuobiao+SIZE*(x1-1)+y1)=='+')return 0;\
		else if(*(zuobiao+SIZE*x1+y1-1)=='U') {gotoxy(0,0);	\
				gotoxy(2*y1,x1);\
				*(zuobiao+SIZE*x1+y1)='*';\
				printf("%c",*(zuobiao+SIZE*x1+y1));\
				y1--;\
...
if(jijie==3||jijie==5){gotoxy(0,30);printf("如果你莫名其妙就没了，请看看规则里秋季的规则吧");}
gotoxy(0,31);
